  ...
  return (
    <>
      <div
        ...
        className={`flex h-screen flex-col justify-between border-r border-emerald-900/30 bg-[#0a0a0a] text-gray-200 transition-all duration-300 ease-in-out ${isExpanded ? "w-64" : "w-20"
          }`}
      >
        ...
        <div>
          ...
          <div className="flex h-16 items-center gap-3 border-b border-emerald-900/30 px-5">
            ...
            {/* App Name */}
            <span
              className={`text-lg font-bold bg-gradient-to-r from-emerald-400 to-green-400 bg-clip-text text-transparent whitespace-nowrap overflow-hidden transition-all duration-300 ${isExpanded ? "opacity-100 w-auto" : "opacity-0 w-0"
                }`}
            >
              CognitoFlow
            </span>
            ...
            {navItems.map(({ name, path, icon: Icon }) => {
              ...
              return (
                <Link
                  key={path}
                  to={path}
                  className={`flex items-center rounded-2xl px-3 py-3 text-sm font-medium transition-all duration-200 relative group ${isExpanded ? "gap-3 justify-start" : "justify-center"
                    } ${active
                ...
                >
                  ...
                  <span
                    className={`whitespace-nowrap overflow-hidden transition-all duration-300 ${isExpanded ? "opacity-100 w-auto" : "opacity-0 w-0"
                      }`}
                  >
                    {name}
                  </span>
        ...
        <div className="border-t border-emerald-900/30 p-3">
          ...
          {user && (
            <div
              className={`mb-3 flex items-center gap-3 rounded-xl bg-[#0a110a]/80 backdrop-blur-sm border border-emerald-900/30 p-3 transition-all duration-300 overflow-hidden ${isExpanded ? "opacity-100 translate-y-0" : "hidden opacity-0"
                }`}
            >
